...
function addPoster(name, age1, description1, age2, description2, imgString, note) {
    ...
    $.ajax({
        type: "post",
        data: paras,
        url: "/yixin/Api/UpLoadImg.ashx?m=" + Math.random(),
        success: function (result) {
            console.log(result)
            //alert(result);
            img = result;
            ...
            $.ajax({
                type: "post",
                data: paras,
                url: "/yixin/Api/AddPoster.ashx?m=" + Math.random(),
                success: function (result) {
                    console.log(result)
                    //alert(result);
                    //用户上传海报的id;
                    ...
                    wxtitle = "我是"+name+"，"+note + '，我只是选择做不一样的自己';
                    wxdesc = wxtitle;
                    wxlink = "http://wx.fractalist.com.cn/yixin/h5/diy/share.html?poterid=" + posterid;
                    //alert("wxlink:" + wxlink);
                    wximgUrl = img;
                    //alert("wximgUrl:" + img);
